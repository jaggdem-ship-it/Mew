...
export class JuiceSystem {
  ...
  // ===== MISSING METHODS CALLED BY ENTITIES =====

  enemyDeath(entity) {
    this.deathBurst(entity.x, entity.y, 0x8B0000, 10, 3);
    this.bloodSplatter(entity.x, entity.y, 6);
    this.screenShake(3, 150);
  }

  bossIntro(sprite, name) {
    this.bossIntroCinematic(sprite, name);
  }

  teleportEffect(x, y) {
    const flash = this.scene.add.circle(x, y, 20, 0x9370DB, 0.6);
    flash.setDepth(100);
    this.scene.tweens.add({
      targets: flash,
      scale: 2,
    ...
    this.deathBurst(x, y, 0x9370DB, 8, 2);
  }

  bossDeath(boss) {
    this.bossDeathCinematic(boss.sprite);
  }
}
