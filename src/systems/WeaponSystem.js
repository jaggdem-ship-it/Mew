...
export class WeaponSystem {
  ...
  update(dt, player, enemies, collisionSystem, pools) {
    ...
    for (let i = this.hydras.length - 1; i >= 0; i--) {
      ...
      h.timer += dt;
      // Hydra bob animation
      h.obj.y = h.y + Math.sin(this.scene.time.now / 300 + h.offset) * 2;
      if (h.timer > 1500) {
        h.timer = 0;
        ...
        if (nearest) {
          ...
          this.scene.juice.muzzleFlash(h.x, h.y, 0xFF4500, 0.4);
        }
      }
      h.lifetime -= dt;
      if (h.lifetime <= 0) {
        this.scene.juice.deathBurst(h.x, h.y, 0xFF6600, 6, 2);
        h.obj.destroy();
    ...
    switch (def.type) {
      case "projectile": {
        ...
        if (key === "teeth") {
          ...
          this.scene.juice.muzzleFlash(player.x, player.y, 0xDDDDDD, 0.4);
          ...
          this.scene.juice.muzzleFlash(player.x, player.y, 0x87CEEB, 0.5);
          ...
          this.scene.juice.muzzleFlash(player.x, player.y, 0xFFFF00, 0.6);
          ...
          for (let i = 0; i < chains && current; i++) {
            ...
            this.scene.juice.spawnTrail(player.x, player.y, 0xFFFF00, 0.3, 100);
            current = collisionSystem.queryNearest(current.x, current.y, 100);
            if (hit.has(current)) break;
          }
        } else if (key === "bone_spirit") {
          const count = scaling.count ?? 1;
          this.scene.juice.muzzleFlash(player.x, player.y, 0xDDDDDD, 0.5);
          for (let i = 0; i < count; i++) {
          ...
          const color = key === "firebolt" ? 0xFF4500 : key === "javelin" ? 0xCCCCCC : 0xFFD700;
          this.scene.juice.muzzleFlash(player.x, player.y, color, 0.4);
      ...
      case "aura": {
        ...
        this.scene.juice.spawnTrail(player.x, player.y, 0xFF4444, 0.5, 200);
        break;
      }
      case "orbit": {
        const count = scaling.count ?? 1;
        for (let i = 0; i < count; i++) {
          ...
          for (const e of nearby) {
            if (e.takeDamage) e.takeDamage(damage, ox, oy);
          }
          const hammer = this.scene.add.image(ox, oy, "holy_bolt");
          hammer.setScale(0.5);
          hammer.setAlpha(0.7);
          hammer.setRotation(a + Math.PI / 2);
          hammer.setDepth(4);
      ...
      case "nova": {
        ...
        this.scene.juice.muzzleFlash(player.x, player.y, 0x32CD32, 0.8);
        break;
      }
      case "summon": {
        const heads = scaling.heads ?? 1;
        this.scene.juice.muzzleFlash(player.x, player.y, 0xFF6600, 0.6);
        for (let i = 0; i < heads; i++) {
          ...
          this.hydras.push({ obj: hObj, x: hx, y: hy, timer: 0, lifetime: 8000, offset: Math.random() * Math.PI * 2 });
        }
        break;
      }
      case "strike": {
        ...
        for (let i = 0; i < bolts; i++) {
          ...
          if (!target) continue;
          // Telegraph
          const telegraph = this.scene.add.circle(target.x, target.y, 8, 0xFFD700, 0.3);
          ...
          this.scene.time.delayedCall(200, () => {
            ...
            strike.setAlpha(0.9);
            strike.setDepth(5);
            this.scene.juice.muzzleFlash(target.x, target.y, 0xFFD700, 1);
            this.scene.tweens.add({
              targets: strike,
              scale: 1,
              alpha: 0,
              ...
              onComplete: () => {
                strike.destroy();
                if (target.takeDamage) target.takeDamage(damage, target.x, target.y);
              }
            });
          });
        }
